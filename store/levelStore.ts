...
const createEntry = (word: string, hiragana: string, meaning: string): VocabEntry => {
  const now = Date.now()
  return {
    word,
    hiragana,
...
export const useVocabStore: UseBoundStore<StoreApi<VocabStore>> = create<VocabStore>((set) => ({
  ...
  reset: () => {
    const entries = Object.fromEntries(
      initialWords.map((w) => [w.fields[0], w.fields[1], w.fields[2], createEntry(w.fields[0], w.fields[1], w.fields[2])])
    )
